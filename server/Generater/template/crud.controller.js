const controllerContentTemp =(modelName,controllerName)=> {
    let ModelName=modelName.toUpperCase();
...
async getAllByPage(req,res,next){
    ...
    try {
        const page = parseInt(req.query.page) || 1; // extract the page number from the request query, default to 1 if not provided
          ...
          const { count, rows } = await ${ModelName}.findAndCountAll({ offset, limit }); // find and count all data based on the pagination parameters
...
module.exports = ${controllerName};
  ...
  module.exports=controllerContentTemp