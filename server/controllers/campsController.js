...
const MaterValues = require('../models/mastervalues');
const Validation=require('../Utlity/Validations/camps.validation')
...
async getAllByPage(req,res,next){
    ...
    try {
        const page = parseInt(req.query.page) || 1; // extract the page number from the request query, default to 1 if not provided
          ...
          const { count, rows } = await CAMPS.findAndCountAll({ offset, limit,...CommmonSchema }); // find and count all data based on the pagination parameters
...
async getAll(req, res, next) {
    ...
    try {
        ...
        const data = await CAMPS.findAll({...CommmonSchema});
  ...
  async getById(req, res, next) {
    ...
    try {
        ...
        const data = await CAMPS.findByPk(req.params.id);
        if (!data) {
  ...
  async create(req, res, next) {
    ...
    // //validation
    const { error, value } = Validation.createSchema.validate(req.body, { abortEarly: false });
    ...
    try {
        ...
        const data = await CAMPS.create(req.body);
  ...
  async update(req, res, next) {
    ...
    try {
        ...
        const data = await CAMPS.update(req.body, { where: { id: req.params.id } });
        if (data[0] === 0) {
          res.status(404).json({ message: 'Data not found' });
        } else {
  ...
  async delete(req, res, next) {
    ...
    try {
        ...
        const data = await CAMPS.destroy({ where: { id: req.params.id } });
        if (data === 0) {
          res.status(404).json({ message: 'Data not found' });
        } else {
