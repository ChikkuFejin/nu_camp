...
async getAllByPage(req,res,next){
    ...
    try {
        const page = parseInt(req.query.page) || 1; // extract the page number from the request query, default to 1 if not provided
          ...
          const { count, rows } = await MASTERVALUES.findAndCountAll({ offset, limit }); // find and count all data based on the pagination parameters
...
async getDropdown(req, res, next) {
    ...
    try {
        ...
        const data = await MASTERVALUES.findAll({attributes:["values","id"],
        where:{
          key:{[Op.eq]:req.params.key},
        }
      });
  ...
  async getAll(req, res, next) {
    ...
    try {
        // Add readAll logic here
        const data = await MASTERVALUES.findAll();
        res.status(200).json({ message: 'Read all successfully', data: data });
      } catch (error) {
        console.error(error);
        next(error); // pass any errors to the error handling middleware
  ...
  async getById(req, res, next) {
    ...
    try {
        ...
        const data = await MASTERVALUES.findByPk(req.params.id);
        if (!data) {
  ...
  async create(req, res, next) {
    ...
    try {
        ...
        const data = await MASTERVALUES.create(req.body);
  ...
  async update(req, res, next) {
    ...
    try {
        ...
        const data = await MASTERVALUES.update(req.body, { where: { id: req.params.id } });
        if (data[0] === 0) {
          res.status(404).json({ message: 'Data not found' });
        } else {
  ...
  async delete(req, res, next) {
    ...
    try {
        ...
        const data = await MASTERVALUES.destroy({ where: { id: req.params.id } });
        if (data === 0) {
          res.status(404).json({ message: 'Data not found' });
        } else {
...
module.exports = mastervaluesController;
