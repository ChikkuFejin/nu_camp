...
async getAllByPage(req,res,next){
    ...
    try {
        const page = parseInt(req.query.page) || 1; // extract the page number from the request query, default to 1 if not provided
          ...
          const { count, rows } = await BEDS.findAndCountAll({ offset, limit }); // find and count all data based on the pagination parameters
...
async getAll(req, res, next) {
  ...
  async getById(req, res, next) {
    ...
    try {
        ...
        const data = await BEDS.findByPk(req.params.id);
        if (!data) {
  ...
  async create(req, res, next) {
    ...
    try {
        ...
        const data = await BEDS.create(req.body);
  ...
  async update(req, res, next) {
    ...
    try {
        ...
        const data = await BEDS.update(req.body, { where: { id: req.params.id } });
        if (data[0] === 0) {
          res.status(404).json({ message: 'Data not found' });
        } else {
  ...
  async delete(req, res, next) {
    ...
    try {
        ...
        const data = await BEDS.destroy({ where: { id: req.params.id } });
        if (data === 0) {
          res.status(404).json({ message: 'Data not found' });
        } else {
